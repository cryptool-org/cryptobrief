...
OutFile "Sunset_${VERSION}_for_JRE1.9.exe"
...
Function controlJava
    ...
    Success:
    StrCpy $JAVA_EXE "$\"$JAVA_HOME\bin\javaw.exe$\" -jar $\"$INSTDIR\sunset_jre9.jar$\" $\"%1$\""
    Goto Done
...
Section -Main SEC0000
    ...
    SetOutPath $INSTDIR
    SetOverwrite on
    File sunset\sunset_jre9.jar
    File sunset\sunset.ico
    File sunset\ffapl.ico
    SetOutPath $SMPROGRAMS\$StartMenuGroup
    SetOutPath $INSTDIR
    CreateShortCut "$DESKTOP\Sunset.lnk" "$INSTDIR\sunset_jre9.jar" " " "$INSTDIR\sunset.ico" 0
    CreateShortcut "$SMPROGRAMS\$StartMenuGroup\Sunset.lnk" "$INSTDIR\sunset_jre9.jar" " " "$INSTDIR\sunset.ico" 0
...
Section /o -un.Main UNSEC0000
    ...
    Delete /REBOOTOK $INSTDIR\sunset_jre9.jar
...
Function runSunset
#Exec "$\"$JAVA_HOME\bin\javaw.exe$\" -jar $\"$INSTDIR\sunset_jre9.jar$\""
