 ...
 *
 */
public class FactorSquareFree implements IPredefinedProcFunc {
	...
	public void execute(IVm interpreter)
		...
		primeFactors = Algorithm.SquareFree(a);
		res = new Object[primeFactors.size()];
		
		i = 0;
 
		//Generates output Array
		for(GaloisField key : primeFactors.keySet() ){
	...
	throws FFaplException {
		...
		//FactorInteger
	    s = new FFaplPreProcFuncSymbol("factorSquareFree", 
	            null,
	            new FFaplArray(new FFaplPolynomial(), 2),
	            ISymbol.FUNCTION);
		s.setProcFunc(new FactorSquareFree());
