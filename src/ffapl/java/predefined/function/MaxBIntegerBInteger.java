...
/**
 * Calculates Maximum of two BigInteger Values
 * @author Alexander Ortner
 * @version 1.0
 *
 */
public class MaxBIntegerBInteger implements IPredefinedProcFunc {
	...
	public void execute(IVm interpreter)
	    ...
	    interpreter.pushStack(new BInteger(a.add(b).add(a.subtract(b).abs()).shiftRight(1), a.getThread()));
	...
	public static void registerProcFunc(ISymbolTable symbolTable)
		...
		//max(a,b)
		s = new FFaplPreProcFuncSymbol("max", 
                null,
                new FFaplInteger(),
                ISymbol.FUNCTION);
		s.setProcFunc(new MaxBIntegerBInteger());
