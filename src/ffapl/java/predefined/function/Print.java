...
public class Print implements IPredefinedProcFunc {
    ...
    public static void registerProcFunc(ISymbolTable symbolTable, FFaplLogger logger)
        ...
        printRecord(symbolTable, logger);
    }
...
//        }
    ...
    private static void printRecord(ISymbolTable symbolTable, FFaplLogger logger) throws FFaplException {
        FFaplPreProcFuncSymbol s;
        s = new FFaplPreProcFuncSymbol("print",
                null,
                null,
        ...
        for (int i = 1; i <= maxArrayDim; i++) {
            ...
            symbolTable.addSymbol(
                    new FFaplSymbol("_t1",
                            null,
                            new FFaplArray(new FFaplRecord(), i),
                            ISymbol.PARAMETER));
            symbolTable.closeScope();
    ...
    public static ISymbol recordName = null;
    public static ISymbol recordScope = null;
    public static IVm  interpreterPrint= null;
    @Override
    public void execute(IVm interpreter)
            throws FFaplAlgebraicException {
        IJavaType a;
        String msg=null;
        interpreterPrint = interpreter;
        if(recordSwitch){
             msg = "RECORD: "+recordName+"="+interpreter.getRecordScope(0,recordScope,recordName);
        }else{
