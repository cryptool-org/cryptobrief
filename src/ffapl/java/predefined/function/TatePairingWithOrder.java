 ...
 *
 */
public class TatePairingWithOrder implements IPredefinedProcFunc {
	...
	public void execute(IVm interpreter)
		...
		c = (IJavaType) interpreter.popStack();
		
		if (b.typeID() == IJavaType.ELLIPTICCURVE && c.typeID() == IJavaType.ELLIPTICCURVE)
		{
			GaloisField pairing = Algorithm.tatePairing((EllipticCurve)c,(EllipticCurve)b, (BigInteger)a);
	...
	public static void registerProcFunc(ISymbolTable symbolTable)
		...
		s = new FFaplPreProcFuncSymbol("TLPairing", 
                ...
                ISymbol.FUNCTION);
		s.setProcFunc(new TatePairingWithOrder());
