...
public class Algorithm {
    ...
    public static BigInteger jacobiSymbol(BigInteger a, BigInteger n) {
        ...
        //define e and a_1 so that a = 2^e * a_1  and a_1 is odd
        BigInteger e = ZERO;
        BigInteger a_1 = a;
        while (a_1.remainder(TWO).equals(ZERO)) {
            e = e.add(ONE);
            a_1 = a_1.divide(TWO);
        }
        //if e is even then set s=1, else set s=1 if n mod 8 = (1 or 7)
        //or set s=-1 if n mod 8 = (3 or 5)
        if (e.mod(TWO).equals(ZERO)) {
            s = ONE;
        } else {
            BigInteger nModEight = n.mod(new BigInteger("8"));
            if (nModEight.equals(ONE) || nModEight.equals(new BigInteger("7"))) {
                s = ONE;
            } else if (nModEight.equals(three) || nModEight.equals(new BigInteger("5"))) {
                s = new BigInteger("-1");
            }
        }
        //if n mod 4 = 3 and a_1 mod 4 = 3 then set s = -s
        ...
        if (a_1.equals(ONE)) return s;
        else return s.multiply(jacobiSymbol(n_1, a_1));
    }

    /**
     * Jacobi Symbol for polynomial rings and galois fields
     *
     * @param f
     * @param g
     * @return
     */
    public static BigInteger jacobiSymbol(PolynomialRC f, PolynomialRC g) throws FFaplAlgebraicException {
        BigInteger alpha, result;
        Prime p;
