...
public class FFaplJavaInterpreterVisitor implements IRetArgVisitor<Object, Object> {
        ...
        try {
            ...
            if (node._node2.ispresent()) {
                ...
                if (s1.getType().typeID() == FFaplTypeCrossTable.FFAPLRECORD) {
                    ...
                    //s1 = (ISymbol) _interpreter.popStack();
    ...
    public String getStringRecord(IVm _interpreter,String nRecord){
        String recordValue = _interpreter.getRecordScope(0,Print.recordName,Print.recordName);
        return recordValue;
    }
    ...
    ISymbol stringCastrecord=null;
    ISymbol recordCastString=null;
    @Override
            ...
            case 3: // ident [ Selector ]
                ...
                s1 = _symbolTable.lookup(s1);
                   ...
                   if (s1.getType().typeID() == FFaplTypeCrossTable.FFAPLRECORD) {
                    ...
                    recordCastString  = s1;
                    stringCastrecord = (ISymbol) argument;
                    try{
         ...
         if (argument != null) {
            if (argument instanceof ISymbol) {
                 ...
                 if (s1.getType().typeID() != FFaplTypeCrossTable.FFAPLBOOLEAN &&
                    ...
                    _interpreter.loadValue(s1.getOffset(), s1.isGlobal());
                        if (s1.getType().typeID() == FFaplTypeCrossTable.FFAPLSTRING
                                && recordCastString != null
                                && stringCastrecord !=null
                              && recordCastString.getType().typeID()==9
                              &&stringCastrecord.getType().typeID()==12) {
                                 _interpreter.pushStack(new JString(getStringRecord(_interpreter,String.valueOf(recordCastString))));
                            stringCastrecord=null;
                            stringCastrecord=null;
                       }else if(s1.getType().typeID() != FFaplTypeCrossTable.FFAPLPRIME) {
                ...
                s1 = _symbolTable.lookuplocal((ISymbol) arguments[0], s1);
                 if (((FFaplNodeOpt) (
                    ...
                    if ((Boolean) arguments[1]) {
                        ...
                        //load
                        if(s1.getType().typeID()==9){
                            Print.recordSwitch =true;
                            Print.recordName=s1;
                        }else{
                            _interpreter.loadValue(s1.getOffset(), s1.isGlobal());
                        }
...
int eCounter=0;
    ...
    public Object visit(FFaplNodeListOpt node, Object argument)
        ...
        if (node.ispresent()) {
             for (final Iterator<INode> e = node.iterator(); e.hasNext(); ) {
                 obj = e.next().accept(this, argument);
                //System.out.println("object e "+ e.next().toString());
                if (obj instanceof LoopState) {
                    if (LoopState.BREAK == obj) {
                        break;
                    }
        ...
        if (node.ispresent()) {
            return node.getNode().accept(this, argument);
                ...
                if (argument!=null){
                    try{
                        arguments = (Object[]) argument;
                        record = ((ISymbol) arguments[0]);
                        if(record.getType().typeID()==9){
                             buildRecord(argument);
                        }
    ...
    public void buildRecord(Object argument){
        Object[] arguments;
        ISymbol record;
        try{
            arguments = (Object[]) argument;
            record = ((ISymbol) arguments[0]);
            if(record.getType().toString() == "Record"){
                Print.recordSwitch = true;
                Print.recordName = record;
                Print.recordScope = record.scope();
            }
        }catch(ClassCastException e){
    ...
    public Object visit(FFaplNodeSequence node, Object argument)
            throws FFaplException {
        Object obj = null;//check the node
        for (final Iterator<INode> e = node.iterator(); e.hasNext(); ) {
            obj = e.next().accept(this, argument);
            if (obj instanceof LoopState) {
                if (LoopState.BREAK == obj) {
